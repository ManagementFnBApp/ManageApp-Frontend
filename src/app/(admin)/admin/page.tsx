            ...
            {loading ? (
              ...
              filtered.map((u) => (
                <tr key={u.user_id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <Badge
                      text={u.isActive ? "Hoạt động" : "Vô hiệu"}
...
function TenantsTab() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(`Xóa tenant "${name}"? Hành động này không thể hoàn tác.`))
      return;
    setDeletingId(id);
    try {
      ...
      alert((e as { message?: string })?.message || "Xóa thất bại");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {tenants.length} cửa hàng trong hệ thống
          </p>
        </div>
        <input
          type="text"
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <EmptyRow cols={7} message="Chưa có tenant nào" />
            ) : (
              filtered.map((t) => (
                <tr key={t.tenant_id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {t.loyal_point_per_unit ?? 0} pt/đv
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      text={t.is_active ? "Hoạt động" : "Tạm dừng"}
...
function AdminsTab() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(
        (e as { message?: string })?.message || "Tạo admin thất bại",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleToggle = async (id: number, current: boolean) => {
    try {
      ...
      setAdmins((prev) =>
        prev.map((a) =>
          a.adminId === id ? { ...a, isActive: updated.isActive } : a,
        ),
      );
    } catch (e: unknown) {
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <EmptyRow cols={7} message="Chưa có admin nào" />
            ) : (
              admins.map((a) => (
                <tr key={a.adminId} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    {a.lastLogin ? (
                      new Date(a.lastLogin).toLocaleString("vi-VN")
                    ) : (
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    ...
                    <button
                      onClick={() => handleDelete(a.adminId, a.email)}
