...
export interface CreateProductPayload {
  categoryId: number;
  productName: string;
  /** Backend nhận uploaded file (field "image") hoặc URL string tùy API */
  image?: string | File;
  barcode?: string;
  description?: string;
  measureUnit?: string;
...
// ===== HELPERS (ảnh) ─────────────────────────────────────────────────────────
/** Trả về URL đầy đủ để hiển thị ảnh (backend có thể trả path hoặc full URL). */
...
// ===== PRODUCT APIs ─────────────────────────────────────────────────────────────
// Backend: GET /products?isActive= | POST /products (multipart) | GET|PATCH|DELETE /products/:id | DELETE /products/:id/hard
...
/**
 * POST /products - Backend dùng FileInterceptor('image'), nhận multipart/form-data.
 * Gửi FormData với field "image" (File) + các field khác (camelCase).
 */
