...
function mapShopProduct(raw: Record<string, unknown>): Product {
  return {
    ...
    sku: raw.image != null ? String(raw.image) : '',
 ...
 */
export const getShopProducts = async (isActive?: boolean): Promise<Product[]> => {
  const res = await apiClient.get('/shop-products');
  const raw = unwrap<unknown>(res.data);
  const arr = Array.isArray(raw) ? raw : [];
  const products = arr.map((item) => mapShopProduct((item as Record<string, unknown>) ?? {}));
 ...
 */
export const createShopProduct = async (payload: CreateProductPayload): Promise<Product> => {
  const categoryId = Number(payload.categoryId);
  if (!Number.isInteger(categoryId) || categoryId <= 0) {
    throw new Error('Danh mục sản phẩm không hợp lệ. Vui lòng chọn danh mục trước.');
  }
  const image =
