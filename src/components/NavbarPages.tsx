...
interface Product {
  ...
  productName: string;
  sku: string;
  importPrice: number; // giá nhập kho - khớp với BE field importPrice
  listPrice: number; // giá bán lẻ - khớp với BE field listPrice
  isActive: boolean;
  categoryId: number;
}
  ...
  const timeline = [
    {
      ...
      title: "Khởi đầu",
      description:
        "ManageApp được thành lập với sứ mệnh số hóa doanh nghiệp Việt",
      icon: "🚀",
    },
    {
      year: "2021",
      ...
      <section className="bg-gradient-to-br from-blue-50 via-white to-blue-50 py-20 px-4">
        <div className="container mx-auto text-center">
          ...
          <p className="text-xl md:text-2xl text-gray-600 max-w-3xl mx-auto mb-8">
            Chúng tôi xây dựng công cụ giúp doanh nghiệp Việt Nam phát triển và
            thành công
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <p className="text-xl text-gray-600 leading-relaxed">
            Chúng tôi tin rằng mọi doanh nghiệp, dù lớn hay nhỏ, đều xứng đáng
            có được công cụ quản lý hiện đại, dễ sử dụng và giá cả phải chăng.
          </p>
        </div>
      </section>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-2">
                {features.map((f) => (
                  <button
                    ...
                    className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-left transition ${activeFeature === f.id ? "bg-blue-50 text-blue-600 font-semibold border-l-4 border-blue-600" : "text-gray-700 hover:bg-gray-100"}`}
                  >
                ...
                >
                  ...
                  <div className="mt-3 flex justify-between items-center">
                    <span className="text-lg font-bold text-blue-600">
                      {new Intl.NumberFormat("vi-VN").format(p.listPrice)} đ
                    </span>
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium ${p.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                    >
...
export function ServicesPage() {
  ...
  const handleSelectPlan = (plan: SubscriptionPlan) => {
    if (!isLoggedIn) {
      router.push(
        `/auth?mode=login&returnUrl=/checkout?subscriptionId=${plan.subscription_id}`,
      );
      return;
    }
    router.push(
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto">
          {isLoading ? (
            ...
            <div
              className={`grid gap-8 ${plans.length <= 2 ? "md:grid-cols-2 max-w-3xl mx-auto" : plans.length === 3 ? "md:grid-cols-3" : "md:grid-cols-2 lg:grid-cols-4"}`}
            >
              {plans.map((plan, index) => {
                ...
                return (
                  <div
                    ...
                    className={`relative bg-white rounded-2xl shadow-lg p-8 hover:shadow-2xl transition-all flex flex-col ${isPopular ? "border-4 border-blue-500 transform scale-105" : "border border-gray-200"}`}
                  >
                    {isPopular && (
                    ...
                    <div className="mt-auto">
                      {userRole === "SHOPOWNER" ? (
                        ...
                        <button
                          ...
                          className={`w-full px-6 py-3 rounded-lg font-semibold transition-all ${isPopular ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-100 text-gray-900 hover:bg-gray-200"}`}
                        >
