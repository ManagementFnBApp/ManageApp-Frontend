...
function CheckoutContent() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setUsername(uname)
      if (!token) {
        router.replace(`/auth?mode=login&returnUrl=/checkout?subscriptionId=${subscriptionId}&packageCode=${packageCode}&price=${price}&billing=${billing}`)
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-2">Thông tin gói không hợp lệ</h2>
          <p className="text-gray-500 mb-6">Vui lòng chọn lại gói từ trang dịch vụ.</p>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-white px-4 pt-20">
        <div className="bg-white rounded-3xl shadow-2xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-6">
            ...
            Tài khoản đã được nâng cấp lên <strong className="text-blue-600">Shop Owner</strong>.
          </p>
  ...
  // === FORM STATE ===
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 py-28 px-4">
        ...
        <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 sticky top-28">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-bold text-gray-900 text-right max-w-[60%]">{packageCode}</span>
                </div>
                <div className="flex justify-between items-start">
                ...
                <div className="border-t border-gray-100 pt-4 flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-blue-600">
                    ...
                    <span className="text-sm font-normal text-gray-500">{billingLabel}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="mt-6 bg-green-50 rounded-xl p-4 flex items-start gap-3">
                ...
                <p className="text-green-700 text-xs leading-relaxed">
                  Thanh toán an toàn & bảo mật. Tài khoản được kích hoạt ngay sau khi thanh toán thành công.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <p className="text-gray-500 text-sm mb-8">Chọn phương thức thanh toán phù hợp với bạn</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {PAYMENT_METHODS.map((method) => (
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${selectedMethod === method.id ? 'text-blue-700' : 'text-gray-700'}`}>
                        {method.label}
                      </span>
                      {selectedMethod === method.id && (
              ...
              <div className="mb-8 bg-yellow-50 border border-yellow-200 rounded-xl p-5">
                ...
                {selectedMethod === 'BANK_TRANSFER' && (
                  <div className="text-sm text-yellow-700 space-y-1">
                    ...
                    <p><strong>Chủ tài khoản:</strong> MANAGE APP CO., LTD</p>
                    <p><strong>Nội dung CK:</strong> MANAGEAPP {username.toUpperCase()}</p>
                  </div>
                )}
                {selectedMethod === 'MOMO' && (
                  <p className="text-sm text-yellow-700">Quét mã QR MoMo hoặc chuyển đến số: <strong>0901 234 567</strong></p>
                )}
                {selectedMethod === 'VNPAY' && (
                  <p className="text-sm text-yellow-700">Bạn sẽ được chuyển đến cổng thanh toán VNPay sau khi nhấn xác nhận.</p>
                )}
                {selectedMethod === 'CASH' && (
                  <p className="text-sm text-yellow-700">Vui lòng đến văn phòng ManageApp để thanh toán tiền mặt. Địa chỉ: 123 Nguyễn Văn Linh, Đà Nẵng.</p>
                )}
              </div>
